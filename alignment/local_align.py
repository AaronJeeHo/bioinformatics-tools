...
class AlignmentGraph:
    ...
    def get_node(self, i, j):
        ...
        :return tuple: Node represented as (row, col)
        """
        curr_row = self.matrix[j]
        return curr_row[i]
...
def main():
    parser = argparse.ArgumentParser(description='Calculate optimal local alignment of two seqs')
    parser.add_argument('sequence_file', help='Input sequence file')
    parser.add_argument('-m', '--match', type=int, help='Match value')
    parser.add_argument('-s', '--mismatch', type=int, required=True, help='Mismatch value')
    parser.add_argument('-d', '--indel', type=int, required=True, help='Indel value')
